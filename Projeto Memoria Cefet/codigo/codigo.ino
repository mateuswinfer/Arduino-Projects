...
unsigned long previousMillis = 0; // Armazena a última vez que o LED foi alternado 
const long interval = 1000; //Intervalo de piscada em milissegundos (1 segundo) 

void setup() {
  // Declaração dos pinos usados
  pinMode(pushG, INPUT); // entrada do botão relacionado ao LED verde
  pinMode(pushR, INPUT); // entrada do botão relacionado ao LED vermelho
  pinMode(pushY, INPUT); // entrada do botão relacionado ao LED amarelo
  pinMode(pushB, INPUT); // entrada do botão relacionado ao LED azul
  pinMode(ledG, OUTPUT); // saída do sinal que acende o LED verde 
  pinMode(ledR, OUTPUT); // saída do sinal que acende o LED
  pinMode(ledY, OUTPUT); // saída do sinal que acende o LED
  pinMode(ledB, OUTPUT); // saída do sinal que acende o LED
//pinMode(buzz, OUTPUT); saída do sinal que ativa o buzzer 
...
      // Se o intervalo desejado for passado, ativa o LED 
...
      int pg = analogRead(pushG);            // Lê os botões de cada luz
